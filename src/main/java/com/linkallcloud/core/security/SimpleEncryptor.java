...
public class SimpleEncryptor implements IEncryptor {
    ...
    public void initKey(String skey) throws Exception {
        if (Strings.isBlank(skey)) {
            throw new BaseException("e.blank.key", "Key must not null.");
        } else {
     ...
     * (non-Javadoc)
     * 
     * @see com.qc.security.IEncryptor#encrypt(java.lang.String)
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.qc.security.IEncryptor#decrypt(java.lang.String)
     */
    public String decrypt(String strMi) {
        try {
            byte[] byteMi = simpleCoder(EncodeUtils.hexDecode(strMi));// hex to byte
     ...
     * (non-Javadoc)
     * 
     * @see com.qc.security.IEncryptor#encryptAndBase64(java.lang.String)
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.qc.security.IEncryptor#decryptAndBase64(java.lang.String)
     */
    public String decryptAndBase64(String src) {
        try {
            byte[] byteMi = simpleCoder(EncodeUtils.base64Decode(src));// hex to byte
            return new String(byteMi, getEncoding());
        } catch (Exception e) {
            log.error(e.getMessage(), e);
        }
